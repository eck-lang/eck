...
use language_core::{
    ...
    ResolvedSubtypeConversion, Scale, TypeId, ValueType,
};
use syntax::{BinaryOperator, ComparisonOperator, Expression, UnaryOperator};
...
impl Compiler<'_> {
    ...
    pub(super) fn compile_expression(
        ...
        match expression {
            ...
            Expression::Variable { name, span } => {
                ...
                Ok(TypedExpression {
                    output: Some(variable.value_type),
            ...
            Expression::Binary {
                ...
                let mut right_operand_type = right_operand.output.ok_or_else(|| {
                    CompileError::new(right_operand.span, "right operand has no value")
                })?;
            ...
            Expression::Comparison {
                ...
                let right_operand_type = right_operand.output.ok_or_else(|| {
                    CompileError::new(right_operand.span, "right comparison operand has no value")
                })?;
            ...
            Expression::Logical {
                ...
                let right_operand_type = right_operand.output.ok_or_else(|| {
                    CompileError::new(right_operand.span, "right logical operand has no value")
                })?;
            ...
            Expression::Pipe {
                ...
                let function_id = self
                    ...
                    .or_else(|error| match error {
                        language_core::CoreError::UnknownFunction(_) => {
    ...
    fn compile_string_conversion(
        ...
        let argument_type = expression.output.ok_or_else(|| {
            CompileError::new(
                expression.span,
        ...
        Ok(TypedExpression {
            ...
            output: Some(ValueType::plain(string_type)),
            span,
        })
    }
}
