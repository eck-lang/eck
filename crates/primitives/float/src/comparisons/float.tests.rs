...
fn compares_finite_float_values() {
    let smaller = Value::new(crate::test_type_id(), -1.25_f32);
    let larger = Value::new(crate::test_type_id(), 2.5_f32);
...
fn preserves_float_special_value_semantics() {
    let positive_zero = Value::new(crate::test_type_id(), 0.0_f32);
    let negative_zero = Value::new(crate::test_type_id(), -0.0_f32);
