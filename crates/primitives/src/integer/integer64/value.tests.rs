...
fn extracts_integer_values_and_rejects_other_representations() {
    let integer = Value::new(crate::integer::integer64::test_type_id(), 42_i64);
    let float = Value::new(crate::integer::integer64::test_type_id(), 42.0_f32);
...
/// Verifies every narrower signed representation widens exactly to `int64`.
#[test]
fn widens_mixed_integer64_operands() {
    let wider_id = crate::integer::integer64::test_type_id();
    let wide = Value::new(wider_id, i64::MAX);
    for (narrow, expected) in [
        ...
        assert_eq!(
            ...
            (expected, i64::MAX, wider_id)
        );
        assert_eq!(
            mixed_operands(&wide, &narrow).unwrap(),
            (i64::MAX, expected, wider_id)
        );
    }
