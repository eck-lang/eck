...
/// Raises an integer to a power, promoting overflowed results to `int128`.
pub(crate) fn power_integer_with_context(
    ...
    match power_integer(lhs, rhs) {
        ...
        Err(error) if is_overflow_error(&error) => {
            promote_overflow_to_int128(context, get(lhs)?, get(rhs)?, BinaryOperator::Power)
        }
        Err(error) => Err(error),
    }
}

/// Raises mixed-width integers after losslessly promoting both operands to `int64`.
...
/// Raises mixed-width integers, promoting overflowed results to `int128`.
pub(crate) fn power_mixed_integer_with_context(
    ...
    match power_mixed_integer(left_operand, right_operand) {
        ...
        Err(error) if is_overflow_error(&error) => {
            ...
            promote_overflow_to_int128(context, left_integer, right_integer, BinaryOperator::Power)
        }
        Err(error) => Err(error),
    }
}
