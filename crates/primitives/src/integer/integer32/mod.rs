...
/// Registers the built-in signed 32-bit integer type and its operators.
pub struct Integer32Extension;

impl Integer32Extension {
    /// Creates an extension for the signed 32-bit integer type.
    pub const fn new() -> Self {
        Self
    }
}

impl Extension for Integer32Extension {
    /// Returns the stable extension name used by the registry.
    fn name(&self) -> &'static str {
        "int32"
    }

    /// Registers `int32` and its numeric semantics.
...
/// Allocates a valid signed 32-bit integer type identifier for isolated operation tests.
#[cfg(test)]
