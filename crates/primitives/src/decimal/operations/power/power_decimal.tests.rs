...
fn calculates_decimal_power_and_rejects_fractional_exponents() {
    let positive = power_decimal(
        &Value::new(crate::decimal::test_type_id(1), Decimal::new(25, 1)),
    ...
    let fractional = power_decimal(
        &Value::new(crate::decimal::test_type_id(1), Decimal::new(2, 0)),
        &Value::new(crate::decimal::test_type_id(1), Decimal::new(15, 1)),
    );
    let zero_to_negative = power_decimal(
        &Value::new(crate::decimal::test_type_id(1), Decimal::ZERO),
        &Value::new(crate::decimal::test_type_id(1), Decimal::NEGATIVE_ONE),
    );
    let overflow = power_decimal(
        &Value::new(crate::decimal::test_type_id(1), Decimal::MAX),
        &Value::new(crate::decimal::test_type_id(1), Decimal::new(2, 0)),
    );
    ...
    assert!(matches!(fractional, Err(CoreError::Runtime(message)) if message.contains("integer")));
    assert!(matches!(zero_to_negative, Err(CoreError::DivisionByZero)));
    assert!(matches!(overflow, Err(CoreError::Runtime(message)) if message.contains("overflow")));
}
