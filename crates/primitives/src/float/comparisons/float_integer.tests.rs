...
fn compares_float_and_integer_in_both_operand_orders() {
    let float = Value::new(crate::float::test_type_id(), 2.5_f32);
    let integer = Value::new(crate::float::test_type_id(), 2_i64);
...
fn preserves_integer_precision_beyond_two_to_the_power_of_twenty_four() {
    let rounded_float = Value::new(crate::float::test_type_id(), 16_777_216.0_f32);
    let exact_integer = Value::new(crate::float::test_type_id(), 16_777_217_i64);
...
fn compares_signed_integer_boundaries_without_rounding() {
    let positive_boundary_float = Value::new(crate::float::test_type_id(), 2_f32.powi(63));
    let maximum_integer = Value::new(crate::float::test_type_id(), i64::MAX);
    let negative_boundary_float = Value::new(crate::float::test_type_id(), -2_f32.powi(63));
    let minimum_integer = Value::new(crate::float::test_type_id(), i64::MIN);
...
fn preserves_special_and_fractional_float_semantics() {
    let zero = Value::new(crate::float::test_type_id(), 0_i64);
    let negative_one = Value::new(crate::float::test_type_id(), -1_i64);
    let smallest_positive_float = Value::new(crate::float::test_type_id(), f32::from_bits(1));
    let negative_fraction = Value::new(crate::float::test_type_id(), -0.5_f32);
    let positive_infinity = Value::new(crate::float::test_type_id(), f32::INFINITY);
