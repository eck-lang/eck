...
use crate::semantic::{
    BinaryOperator, BinaryOperatorDescriptor, ComparisonExecutor, ComparisonOperator,
...
impl<'registry> Runtime<'registry> {
    ...
    fn execute_array_method(
        ...
        match crate::containers::array::apply_end_operation(array_value, method, stored_value)? {
    ...
    /// element expression leaves the array unchanged. The payload owns the bounds
    /// contract and the copy-on-write decision, which preserves the value
    /// semantics of an array shared with another binding instead of aliasing the
    /// change into the other binding.
    fn execute_indexed_assignment(
        ...
        index_dispatch: Option<&crate::ir::TypedIndexDispatch>,
        ...
        crate::containers::array::set_element(array_value, index, element)?;
        Ok(())
    }
