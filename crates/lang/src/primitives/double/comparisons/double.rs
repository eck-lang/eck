...
use crate::semantic::{ComparisonOperator, CoreError, Registry, Value};

use super::{declare_pair, evaluate};

/// Registers every comparison operator for two double operands.
...
/// Produces the IEEE-754 partial ordering of two validated double payloads.
fn compare(left_operand: &Value, right_operand: &Value) -> Result<Option<Ordering>, CoreError> {
    let left_double = left_operand.downcast_ref::<f64>().ok_or_else(|| {
        CoreError::InvalidValueRepresentation("double comparison left operand".into())
    })?;
    let right_double = right_operand.downcast_ref::<f64>().ok_or_else(|| {
        CoreError::InvalidValueRepresentation("double comparison right operand".into())
    })?;

    Ok(left_double.partial_cmp(right_double))
}

/// Returns whether two double operands are equal.
...
/// Returns whether two double operands are different.
...
/// Returns whether the left double operand is less than the right operand.
...
/// Returns whether the left double operand is less than or equal to the right operand.
...
/// Returns whether the left double operand is greater than the right operand.
...
/// Returns whether the left double operand is greater than or equal to the right operand.
