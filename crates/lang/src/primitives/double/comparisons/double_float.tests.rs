...
fn compares_double_and_float_in_both_operand_orders() {
    ...
    let float = Value::new(crate::primitives::double::test_type_id(), 2.5_f32);
...
fn preserves_double_precision_when_comparing_with_float() {
    let double = Value::new(
        crate::primitives::double::test_type_id(),
        f64::from(1.0_f32).next_up(),
    );
    let float = Value::new(crate::primitives::double::test_type_id(), 1.0_f32);
...
fn preserves_float_special_value_semantics() {
    let double = Value::new(crate::primitives::double::test_type_id(), 1.0_f64);
    let nan = Value::new(crate::primitives::double::test_type_id(), f32::NAN);
    let positive_infinity = Value::new(crate::primitives::double::test_type_id(), f32::INFINITY);
