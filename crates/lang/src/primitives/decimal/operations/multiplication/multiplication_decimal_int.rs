use crate::semantic::{CoreError, Value};

use crate::primitives::decimal::operations::{checked_multiplication, decimal_int_operands};

/// Multiplies one decimal and one integer, regardless of operand order.
pub(crate) fn multiplication_decimal_int(
    left_operand: &Value,
    right_operand: &Value,
) -> Result<Value, CoreError> {
    let (left_operand, right_operand, decimal_id) =
        decimal_int_operands(left_operand, right_operand)?;
    Ok(Value::new(
        decimal_id,
        checked_multiplication(left_operand, right_operand)?,
    ))
}

#[cfg(test)]
#[path = "multiplication_decimal_int.tests.rs"]
mod tests;
