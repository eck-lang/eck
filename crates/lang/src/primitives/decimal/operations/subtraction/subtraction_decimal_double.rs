use crate::semantic::{CoreError, Value};

use crate::primitives::decimal::operations::{checked_subtraction, decimal_double_operands};

/// Subtracts a decimal and a double while preserving their source order.
pub(crate) fn subtraction_decimal_double(
    left_operand: &Value,
    right_operand: &Value,
) -> Result<Value, CoreError> {
    let (left_operand, right_operand, decimal_id) =
        decimal_double_operands(left_operand, right_operand)?;
    Ok(Value::new(
        decimal_id,
        checked_subtraction(left_operand, right_operand)?,
    ))
}

#[cfg(test)]
#[path = "subtraction_decimal_double.tests.rs"]
mod tests;
