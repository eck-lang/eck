...
use super::{binary_float::compare_decimal_with_float, declare_pair, evaluate};

/// Registers decimal–float comparisons in both operand orders.
pub(super) fn register(registry: &mut Registry) -> Result<(), CoreError> {
    ...
    declare_pair(registry, "decimal", "float", executors)?;
    declare_pair(registry, "float", "decimal", executors)
}

/// Compares a decimal and a float without converting either value to the other type.
///
...
fn compare(left_operand: &Value, right_operand: &Value) -> Result<Option<Ordering>, CoreError> {
    if let (Some(decimal), Some(float)) = (
        left_operand.downcast_ref::<Decimal>(),
        right_operand.downcast_ref::<f32>(),
    ) {
        return Ok(compare_decimal_with_float(*decimal, *float));
    }
    if let (Some(float), Some(decimal)) = (
        left_operand.downcast_ref::<f32>(),
        right_operand.downcast_ref::<Decimal>(),
    ) {
        return Ok(compare_decimal_with_float(*decimal, *float).map(Ordering::reverse));
    }

    Err(CoreError::InvalidValueRepresentation(
        "decimal and float comparison operands".into(),
    ))
}

/// Returns whether the decimal and float operands are equal.
...
/// Returns whether the decimal and float operands are different.
