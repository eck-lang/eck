...
fn parses_decimal_literal_without_float_conversion() {
    let value = parse("12.500", crate::primitives::decimal::test_type_id(7)).unwrap();

    assert_eq!(value.type_id(), crate::primitives::decimal::test_type_id(7));
    assert_eq!(
...
fn rejects_invalid_decimal_literal() {
    let error = parse("not-a-decimal", crate::primitives::decimal::test_type_id(7))
        .err()
        .unwrap();
