...
pub use integer::Integer128Extension;
...
///
/// ```rust
/// # use eck_lang::semantic::Registry;
/// let mut registry = Registry::new();
/// eck_lang::primitives::register_all(&mut registry)?;
/// # Ok::<(), eck_lang::semantic::CoreError>(())
/// ```
pub fn register_all(registry: &mut Registry) -> Result<(), CoreError> {
    ...
    Integer128Extension::new().register(registry)?;
    BigintExtension::new().register(registry)?;
    integer::register_signed_promotions(registry)?;
    UnsignedIntegerExtension::new().register(registry)?;
    UnsignedInteger8Extension::new().register(registry)?;
