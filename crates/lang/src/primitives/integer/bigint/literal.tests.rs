...
fn parses_arbitrary_precision_integer_literals() {
    let small = parse("42", crate::primitives::integer::bigint::test_type_id()).unwrap();
    let negative = parse("-42", crate::primitives::integer::bigint::test_type_id()).unwrap();
...
fn rejects_non_numeric_literals() {
    for raw_text in ["not-an-integer", "12.5", ""] {
        assert!(matches!(
            parse(raw_text, crate::primitives::integer::bigint::test_type_id()),
