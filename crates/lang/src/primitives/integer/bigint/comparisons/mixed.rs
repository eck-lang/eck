...
use crate::semantic::{ComparisonExecutor, ComparisonOperator, CoreError, Registry, Value};

use super::{declare_pair, evaluate};
use crate::primitives::integer::bigint::value::mixed_operands;

/// Declares mixed comparisons with a `bigint` operand in both operand orders.
pub(super) fn register(registry: &mut Registry) -> Result<(), CoreError> {
    for narrower_type_name in ["int8", "int16", "int32", "int64", "int128"] {
        let executors = executors();
        declare_pair(registry, narrower_type_name, "bigint", executors)?;
        declare_pair(registry, "bigint", narrower_type_name, executors)?;
    }
    Ok(())
}
...
/// Produces the exact ordering after promoting the narrower integer to `bigint`.
