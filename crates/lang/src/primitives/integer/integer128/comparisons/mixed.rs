...
use crate::primitives::integer::integer128::value::mixed_operands;

/// Declares mixed comparisons with an `int128` operand in both operand orders.
pub(super) fn register(registry: &mut Registry) -> Result<(), CoreError> {
    for narrower_type_name in ["int8", "int16", "int32", "int64"] {
        let executors = executors();
        declare_pair(registry, narrower_type_name, "int128", executors)?;
        declare_pair(registry, "int128", narrower_type_name, executors)?;
    }
    Ok(())
}
...
/// Produces the exact ordering after promoting the narrower integer to `int128`.
