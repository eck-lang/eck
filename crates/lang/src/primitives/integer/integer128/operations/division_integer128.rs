...
/// Divides two integers, promoting overflowed results to `bigint`.
///
...
pub(crate) fn division_integer_with_context(
    ...
    match division_integer(lhs, rhs) {
        ...
        Err(error) if is_overflow_error(&error) => {
            promote_overflow_to_bigint(context, get(lhs)?, get(rhs)?, BinaryOperator::Division)
        }
        Err(error) => Err(error),
    }
}

/// Divides mixed-width integers after losslessly promoting both operands to `int128`.
...
/// Divides mixed-width integers, promoting overflowed results to `bigint`.
///
/// The runtime prefers this registry-aware implementation over
/// `division_mixed_integer`; zero divisors and operand extraction errors still
/// report without promotion while `int128` overflow recomputes with arbitrary
/// precision.
...
#[cfg(test)]
#[path = "division_integer128.tests.rs"]
mod tests;
