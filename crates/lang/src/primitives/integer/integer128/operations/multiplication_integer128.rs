...
use crate::primitives::integer::integer128::value::{
...
/// Multiplies two integers, promoting overflowed results to `bigint`.
///
/// The runtime prefers this registry-aware implementation over
/// `multiplication_integer`; overflow recomputes with arbitrary precision
/// while invalid representations still report errors.
pub(crate) fn multiplication_integer_with_context(
    ...
    match multiplication_integer(lhs, rhs) {
        Ok(value) => Ok(value),
        Err(error) if is_overflow_error(&error) => promote_overflow_to_bigint(
            context,
            get(lhs)?,
            get(rhs)?,
...
/// Multiplies mixed-width integers after losslessly promoting both operands to `int128`.
...
/// Multiplies mixed-width integers, promoting overflowed results to `bigint`.
///
/// The runtime prefers this registry-aware implementation over
/// `multiplication_mixed_integer`; operand extraction errors still report
/// without promotion while `int128` overflow recomputes with arbitrary
/// precision.
...
#[cfg(test)]
#[path = "multiplication_integer128.tests.rs"]
mod tests;
