use crate::semantic::{BinaryOperator, ComparisonOperator, CoreError, Extension, Registry, Value};
...
/// Verifies the registry exposes the semantic signed widening order directly.
#[test]
fn exposes_signed_integer_widening_order_without_unsigned_types() {
    let mut registry = Registry::new();
    crate::register_all(&mut registry).unwrap();

    let widening = registry
        .signed_integer_widening_types()
        .into_iter()
        .map(|type_id| registry.type_name(type_id))
        .collect::<Vec<_>>();

    assert_eq!(
        widening,
        vec!["int8", "int16", "int32", "int64", "int128", "bigint"]
    );
}
