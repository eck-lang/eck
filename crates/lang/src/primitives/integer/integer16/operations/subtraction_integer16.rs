...
/// Subtracts two integers, promoting overflowed results to `int32`.
///
...
pub(crate) fn subtraction_integer_with_context(
    ...
    match subtraction_integer(lhs, rhs) {
        ...
        Err(error) if is_overflow_error(&error) => {
            promote_overflow_to_int32(context, get(lhs)?, get(rhs)?, BinaryOperator::Subtraction)
        }
        Err(error) => Err(error),
    }
}

/// Subtracts mixed-width integers after losslessly promoting both operands to `int16`.
...
/// Subtracts mixed-width integers, promoting overflowed results to `int32`.
///
/// The runtime prefers this registry-aware implementation over
/// `subtraction_mixed_integer`; operand extraction errors still report without
/// promotion while `int16` overflow recomputes with wider precision.
...
#[cfg(test)]
#[path = "subtraction_integer16.tests.rs"]
mod tests;
