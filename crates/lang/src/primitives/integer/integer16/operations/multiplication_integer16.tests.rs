...
fn multiplies_integers_and_rejects_overflow() {
    let lhs = Value::new(crate::primitives::integer::integer16::test_type_id(), 6_i16);
    let rhs = Value::new(crate::primitives::integer::integer16::test_type_id(), 7_i16);
...
/// Verifies context-aware mixed multiplication promotes `int16` overflow to `int32`.
#[test]
fn promotes_overflowed_mixed_context_multiplication_to_int32() {
    let mut registry = Registry::new();
    crate::register_all(&mut registry).unwrap();
    let configuration = registry.default_runtime_configuration();
    let context = ExecutionContext::new(&registry, &configuration);
    ...
    let operator = registry
        .resolve_binary_operator(BinaryOperator::Multiplication, integer16_id, integer8_id)
        .unwrap();
    let descriptor = registry.operator(operator).unwrap();
