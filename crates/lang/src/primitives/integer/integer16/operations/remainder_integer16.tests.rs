...
fn calculates_integer_remainder_and_rejects_zero_and_overflow() {
    ...
    let rhs = Value::new(crate::primitives::integer::integer16::test_type_id(), 5_i16);
    let zero = Value::new(crate::primitives::integer::integer16::test_type_id(), 0_i16);
...
/// Verifies context-aware mixed remainder promotes the `MIN % -1` overflow to `int32`.
#[test]
fn promotes_overflowed_mixed_context_remainder_to_int32() {
    let mut registry = Registry::new();
    crate::register_all(&mut registry).unwrap();
    let configuration = registry.default_runtime_configuration();
    let context = ExecutionContext::new(&registry, &configuration);
    ...
    let operator = registry
        .resolve_binary_operator(BinaryOperator::Remainder, integer16_id, integer8_id)
        .unwrap();
    let descriptor = registry.operator(operator).unwrap();
