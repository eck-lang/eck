...
/// Adds two integers, promoting overflowed results to `int32`.
///
...
pub(crate) fn addition_integer_with_context(
    ...
    match addition_integer(lhs, rhs) {
        ...
        Err(error) if is_overflow_error(&error) => {
            promote_overflow_to_int32(context, get(lhs)?, get(rhs)?, BinaryOperator::Addition)
        }
        Err(error) => Err(error),
    }
}

/// Adds mixed-width integers after losslessly promoting both operands to `int16`.
...
/// Adds mixed-width integers, promoting overflowed results to `int32`.
///
/// The runtime prefers this registry-aware implementation over
...
#[cfg(test)]
#[path = "addition_integer16.tests.rs"]
mod tests;
