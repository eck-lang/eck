...
/// Multiplies two integers, promoting overflowed results to `int64`.
///
...
pub(crate) fn multiplication_integer_with_context(
    ...
    match multiplication_integer(lhs, rhs) {
        Ok(value) => Ok(value),
        Err(error) if is_overflow_error(&error) => promote_overflow_to_int64(
            context,
            get(lhs)?,
            get(rhs)?,
...
/// Multiplies mixed-width integers after losslessly promoting both operands to `int32`.
...
/// Multiplies mixed-width integers, promoting overflowed results to `int64`.
///
/// The runtime prefers this registry-aware implementation over
/// `multiplication_mixed_integer`; operand extraction errors still report
/// without promotion while `int32` overflow recomputes with wider precision.
...
#[cfg(test)]
#[path = "multiplication_integer32.tests.rs"]
mod tests;
