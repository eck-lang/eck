...
/// because overflow promotion resolves `int64` by name.
...
fn adds_integers_and_rejects_overflow() {
    ...
    let one = Value::new(crate::primitives::integer::integer32::test_type_id(), 1_i32);
...
fn adds_promoted_integer16_operands_as_integer32() {
    let wider_id = crate::primitives::integer::integer32::test_type_id();
    let narrower_id = crate::primitives::integer::integer16::test_type_id();
...
/// Verifies context-aware addition keeps in-range results as `int32`.
#[test]
fn keeps_in_range_context_addition_as_integer32() {
    ...
    let lhs = Value::new(integer32_id, 15_i32);
    let rhs = Value::new(integer32_id, 27_i32);
    let operator = registry
        .resolve_binary_operator(BinaryOperator::Addition, integer32_id, integer32_id)
        .unwrap();
    let descriptor = registry.operator(operator).unwrap();

    let result = descriptor.context_execute.unwrap()(&context, &lhs, &rhs).unwrap();

    assert_eq!(result.type_id(), integer32_id);
    assert_eq!(*result.downcast_ref::<i32>().unwrap(), 42);
}

/// Verifies context-aware addition promotes `int32` overflow to `int64`.
#[test]
fn promotes_overflowed_context_addition_to_int64() {
    let (registry, configuration) = registered_registry_with_configuration();
    let context = ExecutionContext::new(&registry, &configuration);
    let integer32_id = registry.type_by_name("int32").unwrap();
...
/// Verifies context-aware mixed addition promotes `int32` overflow to `int64`.
#[test]
fn promotes_overflowed_mixed_context_addition_to_int64() {
    let (registry, configuration) = registered_registry_with_configuration();
    let context = ExecutionContext::new(&registry, &configuration);
    let integer32_id = registry.type_by_name("int32").unwrap();
    ...
    let operator = registry
        .resolve_binary_operator(BinaryOperator::Addition, integer32_id, integer16_id)
        .unwrap();
    let descriptor = registry.operator(operator).unwrap();
