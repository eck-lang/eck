...
fn calculates_integer_remainder_and_rejects_zero_and_overflow() {
    ...
    let rhs = Value::new(crate::primitives::integer::integer32::test_type_id(), 5_i32);
    let zero = Value::new(crate::primitives::integer::integer32::test_type_id(), 0_i32);
...
/// Verifies context-aware mixed remainder promotes the `MIN % -1` overflow to `int64`.
#[test]
fn promotes_overflowed_mixed_context_remainder_to_int64() {
    let mut registry = Registry::new();
    crate::register_all(&mut registry).unwrap();
    let configuration = registry.default_runtime_configuration();
    let context = ExecutionContext::new(&registry, &configuration);
    ...
    let operator = registry
        .resolve_binary_operator(BinaryOperator::Remainder, integer32_id, integer16_id)
        .unwrap();
    let descriptor = registry.operator(operator).unwrap();
