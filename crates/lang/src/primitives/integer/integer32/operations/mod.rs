mod addition_integer32;
mod division_integer32;
mod multiplication_integer32;
mod power_integer32;
mod remainder_integer32;
mod subtraction_integer32;

use crate::semantic::{BinaryOperator, ContextBinaryOperatorExecutor, CoreError, Registry, TypeId};
...
use self::{
    ...
    subtraction_integer32::{
        subtraction_integer, subtraction_integer_with_context, subtraction_mixed_integer,
        subtraction_mixed_integer_with_context,
    },
};

/// Registers every binary arithmetic operator for two `int32` operands.
///
/// Same-type operators keep `int32` as their static result while their
/// context-aware override promotes overflowed results to `int64` at runtime.
...
/// Registers every mixed-width operator whose static result is `int32`.
///
/// Mixed operators keep `int32` as their static result while their
/// context-aware override promotes overflowed results to `int64` at runtime.
pub(crate) fn register_promotions(
    ...
    for narrower_id in narrower_ids {
        for (operator, execute, execute_with_context) in [
            (
                BinaryOperator::Addition,
                addition_mixed_integer as crate::semantic::BinaryOperatorExecutor,
...
fn register_mixed_pair(
    ...
    narrower_id: TypeId,
    wider_id: TypeId,
    execute: crate::semantic::BinaryOperatorExecutor,
