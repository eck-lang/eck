...
use crate::primitives::integer::integer32::value::{
...
/// Calculates the integer remainder, promoting overflowed results to `int64`.
///
...
pub(crate) fn remainder_integer_with_context(
    ...
    match remainder_integer(lhs, rhs) {
        ...
        Err(error) if is_overflow_error(&error) => {
            promote_overflow_to_int64(context, get(lhs)?, get(rhs)?, BinaryOperator::Remainder)
        }
        Err(error) => Err(error),
    }
}

/// Calculates a mixed-width remainder after promoting both operands to `int32`.
...
/// Calculates a mixed-width remainder, promoting overflowed results to `int64`.
///
/// The runtime prefers this registry-aware implementation over
/// `remainder_mixed_integer`; zero divisors and operand extraction errors
/// still report without promotion while `int32` overflow recomputes with wider
/// precision.
...
#[cfg(test)]
#[path = "remainder_integer32.tests.rs"]
mod tests;
