...
/// Verifies context-aware mixed subtraction promotes `int32` overflow to `int64`.
#[test]
fn promotes_overflowed_mixed_context_subtraction_to_int64() {
    let mut registry = Registry::new();
    crate::register_all(&mut registry).unwrap();
    let configuration = registry.default_runtime_configuration();
    let context = ExecutionContext::new(&registry, &configuration);
    ...
    let operator = registry
        .resolve_binary_operator(BinaryOperator::Subtraction, integer32_id, integer16_id)
        .unwrap();
    let descriptor = registry.operator(operator).unwrap();
