...
/// Divides two integers, promoting overflowed results to `int64`.
///
...
pub(crate) fn division_integer_with_context(
    ...
    match division_integer(lhs, rhs) {
        ...
        Err(error) if is_overflow_error(&error) => {
            promote_overflow_to_int64(context, get(lhs)?, get(rhs)?, BinaryOperator::Division)
        }
        Err(error) => Err(error),
    }
}

/// Divides mixed-width integers after losslessly promoting both operands to `int32`.
...
/// Divides mixed-width integers, promoting overflowed results to `int64`.
///
/// The runtime prefers this registry-aware implementation over
/// `division_mixed_integer`; zero divisors and operand extraction errors still
/// report without promotion while `int32` overflow recomputes with wider
/// precision.
...
#[cfg(test)]
#[path = "division_integer32.tests.rs"]
mod tests;
