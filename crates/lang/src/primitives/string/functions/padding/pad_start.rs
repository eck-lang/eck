use crate::semantic::{CoreError, ExecutionContext, Value};

use crate::primitives::string::value::get;

/// Pads the string receiver on the start to reach `target_length` using `pad_string`.
///
...
pub(crate) fn pad_start(
    ...
    if arguments.len() != 3 {
        return Err(CoreError::Runtime(
            "pad_start expects a string receiver, a target length, and a pad string".into(),
        ));
    }
    let text = get(&arguments[0])?;
    let target_length = extract_integer(&arguments[1], "pad_start target length")?;
    let pad_string = get(&arguments[2])?;
    if target_length < 0 {
        return Err(CoreError::Runtime(
            "pad_start target length must be non-negative".into(),
        ));
    }
    ...
    if pad_string.is_empty() {
        return Err(CoreError::Runtime(
            "pad_start pad string must not be empty".into(),
        ));
    }
    let needed = target_length - char_count;
    let pad = build_pad(pad_string, needed);
    let transformed = format!("{pad}{text}");
