...
use crate::semantic::configuration::RegisteredTypeConfiguration;
use crate::semantic::{
    ArrayValueFormatter, ConfigurationDescriptor, ConfigurationValue, CoreError,
    RuntimeConfiguration, SemanticType, TypeConfigurationDescriptor, TypeId, Value,
};

use super::Registry;

impl Registry {
    /// Registers the one formatter responsible for array payloads.
    pub fn register_array_formatter(
        &mut self,
        formatter: ArrayValueFormatter,
    ) -> Result<(), CoreError> {
        if self.array_formatter.is_some() {
            return Err(CoreError::DuplicateArrayFormatter);
        }
        self.array_formatter = Some(formatter);
        Ok(())
    }
    ...
    pub fn format_value_with_configuration(
        ...
        if let SemanticType::Array(array_type) = value.semantic_type() {
            let formatter = self
                .array_formatter
