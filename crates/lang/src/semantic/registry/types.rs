...
/// Names the signed integer representations in their semantic widening order.
///
/// This is deliberately independent of registry allocation order and aliases:
/// `int` names `int64`, but it is not a separate representation in the
/// progression, and unsigned integers never participate in adaptive `int`
/// storage.
const SIGNED_INTEGER_WIDENING_NAMES: &[&str] =
...
impl Registry {
    ...
    /// Returns the registered signed integer representations in widening order.
    ///
    /// The returned list is a snapshot of the canonical implementations that
    /// exist in this registry. It never includes aliases or unsigned integer
    /// types, so adaptive integer literal selection does not depend on type
    /// spelling, allocation order, or unrelated extensions.
    pub fn signed_integer_widening_types(&self) -> Vec<TypeId> {
        SIGNED_INTEGER_WIDENING_NAMES
            .iter()
            .filter_map(|name| self.type_by_name(name))
            .filter(|type_id| {
                self.types
                    .get(type_id)
                    .is_some_and(|descriptor| descriptor.is_integer)
            })
            .collect()
    }
    ...
    pub fn evaluate_boolean(&self, value: &Value) -> Result<bool, CoreError> {
        ...
        let actual = match value.semantic_type() {
            SemanticType::Scalar(actual) => actual,
