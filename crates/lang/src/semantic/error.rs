...
pub enum CoreError {
    ...
    #[error("an array value formatter is already registered")]
    DuplicateArrayFormatter,
    #[error("no array value formatter is registered")]
    MissingArrayFormatter,
    #[error("array element `{value}` cannot be represented as `{element}`")]
    ArrayElementNotRepresentable { value: String, element: String },
