...
use crate::semantic::{BinaryOperator, ComparisonOperator, Extension, Registry, Scale, ValueType};

/// Builds a registry with frequency measures registered.
fn registry() -> Registry {
    let mut registry = Registry::new();
    IntegerExtension.register(&mut registry).unwrap();
    DecimalExtension.register(&mut registry).unwrap();
    FrequencyMeasureExtension.register(&mut registry).unwrap();
    registry
}

/// Verifies that the frequency extension registers every unit with its canonical suffix.
#[test]
fn registers_the_frequency_measure_extension_and_canonical_suffixes() {
    let registry = registry();

    assert_eq!(FrequencyMeasureExtension.name(), "frequency-measure");
    for unit in UNITS {
...
fn conversion_to_a_coarser_unit_promotes_integers_to_decimal() {
    ...
    let gigahertz = registry.subtype_by_suffix("ghz").unwrap();

    let conversion = registry
        .resolve_subtype_conversion(ValueType::qualified(int, hertz), gigahertz)
        .unwrap();
...
#[test]
fn frequency_arithmetic_supports_same_and_mixed_units_and_plain_scalars() {
    let registry = registry();
    let int = registry.type_by_name("int").unwrap();
    let hertz = registry.subtype_by_suffix("hz").unwrap();
    let kilohertz = registry.subtype_by_suffix("khz").unwrap();
    ...
    for operator in [
        ...
        let same_unit = registry
            ...
            .unwrap();
        assert_eq!(same_unit.output, ValueType::qualified(int, hertz));

        let mixed_units = registry
            .resolve_binary_operation(
                operator,
                ValueType::qualified(int, kilohertz),
                ValueType::qualified(int, hertz),
            )
            .unwrap();
        assert_eq!(mixed_units.output, ValueType::qualified(int, hertz));
    ...
    for (operator, left, right) in [
        ...
        let resolution = registry
            ...
            .unwrap();
        assert_eq!(resolution.output, ValueType::qualified(int, hertz));
    }
...
/// Verifies that multiplying two frequencies and mixing frequency with other dimensions are rejected.
#[test]
fn multiplying_two_frequencies_and_mixing_frequency_with_other_dimensions_are_rejected() {
...
/// Verifies that mixed frequency comparisons scale the coarser operand to the finer unit.
#[test]
fn mixed_frequency_comparisons_scale_to_the_finer_unit() {
    let mut registry = Registry::new();
    IntegerExtension.register(&mut registry).unwrap();
    DecimalExtension.register(&mut registry).unwrap();
    ...
    let resolution = registry
        .resolve_comparison_operation(
            ...
            ValueType::qualified(integer, kilohertz),
            ValueType::qualified(integer, hertz),
        )
        .unwrap();
