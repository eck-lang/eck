use crate::semantic::{
    BinaryOperator, CoreError, Registry, SemanticType, TypeDescriptor, Value, ValueType,
};
use crate::syntax::{Expression, Program, Span, Statement};

use crate::{TypedExpressionKind, TypedStatement};

use crate::compile;

fn parse_number(raw_text: &str, type_id: crate::semantic::TypeId) -> Result<Value, CoreError> {
...
fn negative_integer_power_uses_the_default_fractional_type() {
    ...
    let program = Program {
        statements: vec![Statement::VariableDeclaration {
            ...
            expression: Expression::Binary {
                operator: crate::syntax::BinaryOperator::Power,
    ...
    let TypedStatement::VariableDeclaration { semantic_type, .. } = statement else {
        panic!("expected a variable declaration");
    };

    assert_eq!(
        *semantic_type,
        SemanticType::Scalar(ValueType::plain(decimal))
    );
}
...
fn multiplication_by_literal_true_is_removed_from_the_typed_program() {
    ...
    let program = Program {
        statements: vec![Statement::VariableDeclaration {
            ...
            type_name: "int".into(),
            expression: Expression::Binary {
                operator: crate::syntax::BinaryOperator::Multiplication,
                left_operand: Box::new(Expression::Number {
                    raw_text: "7".into(),
                    suffix: None,
                    span: span(),
